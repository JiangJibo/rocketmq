...
public class AdminBrokerProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request) throws RemotingCommandException {
        switch (request.getCode()) {
            ...
            case RequestCode.LOCK_BATCH_MQ:              //有序消费模式,获取指定MessageQueue的分布式锁
    ...
    private HashMap<String, String> prepareRuntimeInfo() {
        ...
        if (this.brokerController.getMessageStore() instanceof DefaultMessageStore) {
            ...
            if (defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
                runtimeInfo.put("remainHowManyDataToCommit",
                    MixAll.humanReadableByteCount(defaultMessageStore.getCommitLog().remainHowManyDataToCommit(), false));
            }
        ...
        if (commitLogDir.exists()) {
            runtimeInfo.put("commitLogDirCapacity", String.format("Total : %s, Free : %s.", MixAll.humanReadableByteCount(commitLogDir.getTotalSpace(), false),
